class MyInterpreter:
    ...
    def _check_borrow(self, value_expr, borrow_type, env, borrow_tracker):
        ...
        # Extract the variable being borrowed (and also index)
        borrowed_var, index = self._extract_borrowed_var_and_index(value_expr)
        
        if borrowed_var:
            borrow_tracker.add_borrow(borrowed_var, is_mut, index)
    ...
    def _extract_borrowed_var(self, expr):
        var, _ = self._extract_borrowed_var_and_index(expr)
        return var
    
    def _extract_borrowed_var_and_index(self, expr):
        expr = str(expr).strip()
        index = None
        ...
        if '&shared' in expr or '&mut' in expr:
            ...
            if '[' in expr and ']' in expr:
                ...
                try:
                    ...
                    # Index is a variable or expression, we can't track it
                    index = None
...
class BorrowTracker:
    def __init__(self):
        # var_name -> list of (borrow_type, index) tuples
        # borrow_type is 'mut' or 'shared'
        # index is None for whole variable, or an integer for array index
        self.borrows = {}
    
    def add_borrow(self, var_name, is_mut, index=None):
        if var_name not in self.borrows:
            self.borrows[var_name] = []
        
        current_borrows = self.borrows[var_name]
        borrow_type = 'mut' if is_mut else 'shared'
        
        # Check for conflicts
        if is_mut:
            # Mutable borrow conflicts with:
            # 1. Any borrow of the whole variable (index=None)
            # 2. Any borrow at the same index
            for existing_type, existing_index in current_borrows:
                if existing_index is None:
                    # Whole variable is borrowed
                    raise Exception(f"Cannot create mutable reference to '{var_name}': already borrowed")
                if index is None:
                    # Trying to borrow whole variable, but parts are borrowed
                    raise Exception(f"Cannot create mutable reference to '{var_name}': already borrowed")
                if existing_index == index:
                    # Same index is already borrowed
                    raise Exception(f"Cannot create mutable reference to '{var_name}[{index}]': already borrowed")
            # If index is different from all existing indices, allow it
            self.borrows[var_name].append((borrow_type, index))
        else:
            # Shared borrow conflicts with:
            # 1. Any mutable borrow of the whole variable
            # 2. Any mutable borrow at the same index
            for existing_type, existing_index in current_borrows:
                if existing_type == 'mut':
                    if existing_index is None or index is None:
                        raise Exception(f"Cannot create shared reference to '{var_name}': already mutably borrowed")
                    if existing_index == index:
                        raise Exception(f"Cannot create shared reference to '{var_name}[{index}]': already mutably borrowed")
            self.borrows[var_name].append((borrow_type, index))
    
    def enter_scope(self):
        new_tracker = BorrowTracker()
        # Deep copy the borrows
        for var, borrows in self.borrows.items():
            new_tracker.borrows[var] = borrows.copy()
        return new_tracker
